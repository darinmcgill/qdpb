...
bool Parser::get_string(int field_number, byte_p &ptr, size_t &size,
                        int index) const {
...
inline char to_hex(int c) {
  return static_cast<char>(c < 10 ? c + '0' : c + 'A' - 10);
}

std::string repr(std::string input) {
  std::stringstream ss;
