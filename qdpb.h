...
enum WireType { VARINT = 0, DOUBLE = 1, PASCAL = 2, SINGLE = 5 };
...
public:
  ...
  template <class T> T get_value(int field_number, int index = -1) const {
    misc_t p = get_misc(field_number, index);
    return *reinterpret_cast<T *>(&p);
  }

  bool get_string(int field_number, byte_p &ptr, size_t &size,
                  int index = -1) const;
